...
class ArbitrageStrategy(BaseStrategy):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.config = (
            ...
            if config and not isinstance(config, ArbitrageConfig)
            else (config or ArbitrageConfig())
        )
        self.position = None
        self.stop_loss = None
    ...
    def _setup_logger(self):
        """Настройка логгера"""
        logger.add(
            f"{self.config.log_dir}/arbitrage_strategy_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
            rotation="1 day",
            retention="7 days",
            level="INFO",
        )
    ...
    def _check_basic_conditions(
        ...
        try:
            ...
            # Проверка спреда
            if not (self.config.min_spread <= spread <= self.config.max_spread):
                return False

            # Проверка отклонения от среднего
            if (
                abs(spread - spread_stats["mean"])
                > spread_stats["std"] * self.config.spread_std_multiplier
            ):
                return False
            ...
            # Проверка исполнения
            if execution["slippage"] > self.config.max_slippage:
                return False

            if execution["fill_ratio"] < self.config.min_fill_ratio:
                return False
    ...
    def _calculate_spread(self, data: pd.DataFrame) -> float:
        """
        Расчет спреда между инструментами.
    ...
    def _generate_entry_signal(
        ...
        try:
            ...
            if spread > spread_stats["mean"] + spread_stats["std"]:
                ...
                stop_loss = current_price * (1 - self.config.max_loss_threshold)
                take_profit = current_price * (1 + self.config.min_profit_threshold)
