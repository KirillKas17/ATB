...
class StatisticalArbitrageStrategy(BaseStrategy):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        """
        Инициализация стратегии статистического арбитража.
    ...
    def _apply_risk_parity(self, position_size: float, analysis: Dict[str, Any]) -> float:
        ...
        try:
            ...
            return position_size * risk_parity_size
    ...
    def _calculate_half_life(self, spread: pd.Series) -> int:
        ...
        try:
            ...
            # Расчет коэффициента
            beta = np.cov(spread_lag, spread_ret)[0, 1] / np.var(spread_lag)
        ...
        try:
            ...
            for param, values in params.items():
                if isinstance(values[0], int):
                    space.append(Integer(min(values), max(values), name=param))
                else:
                    space.append(Real(min(values), max(values), name=param))

            # Функция для оптимизации
            def objective(x):
                param_dict = dict(zip(params.keys(), x))
    ...
    def _grid_optimization(
        ...
        try:
            ...
            if len(param_grid) > 1000:
                param_grid = np.random.choice(param_grid, 1000, replace=False)
    ...
    def _calculate_adaptive_z_score(self, returns: pd.Series, window: int) -> float:
        ...
        try:
            if len(returns) < window:
                return self.arb_config.z_score_threshold
    ...
    def _calculate_adaptive_correlation(self, returns: pd.Series, window: int) -> float:
        ...
        try:
            if len(returns) < window:
                return self.arb_config.min_correlation
    ...
    def _calculate_drawdown_duration(self, data: pd.DataFrame) -> int:
        ...
        try:
            ...
            # Расчет продолжительности просадки
            drawdown_duration = drawdown.apply(lambda x: (x < 0).sum()).max()
