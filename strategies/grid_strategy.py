...
class GridStrategy(BaseStrategy):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.config = (
            GridConfig(**config)
            if config and not isinstance(config, GridConfig)
            else (config or GridConfig())
        )
        self.position = None
        self.grid_levels = {}
        self.active_orders = {}
    ...
    def _setup_logger(self):
        """Настройка логгера"""
        logger.add(
            f"{self.config.log_dir}/grid_strategy_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
            rotation="1 day",
            retention="7 days",
            level="INFO",
        )
    ...
    def _update_grid_levels(self, current_price: float):
        ...
        try:
            ...
            if len(self.grid_levels) < self.config.max_grid_levels:
                base_price = current_price * (1 + self.config.grid_shift)

                for i in range(self.config.grid_levels):
                    level = base_price * (1 + i * self.config.grid_spacing)
    ...
    def _calculate_position_size(self, price: float, volatility: float) -> float:
        ...
        try:
            ...
            base_size = self.config.grid_volume
