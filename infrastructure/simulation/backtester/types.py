...
class MarketData:
    ...
    def to_domain_market_data(self) -> DomainMarketData:
        """Преобразование к доменной модели MarketData."""
        return DomainMarketData(
            symbol=Symbol(self.symbol),
            timestamp=TimestampValue(self.timestamp),
            open=DomainPrice(Decimal(str(self.open)), Currency.USD),  # Добавляем валюту
            high=DomainPrice(Decimal(str(self.high)), Currency.USD),
            low=DomainPrice(Decimal(str(self.low)), Currency.USD),
            close=DomainPrice(Decimal(str(self.close)), Currency.USD),
            volume=DomainVolume(Decimal(str(self.volume)), Currency.USD),
...
class Signal:
    ...
    def to_domain_signal(self) -> DomainSignal:
        """Преобразование к доменной модели Signal."""
        return DomainSignal(
            id=str(uuid4()),  # Генерируем ID
            signal_type=DomainSignalType.BUY if self.signal_type == SignalType.ENTRY else DomainSignalType.SELL,
            confidence=self.confidence,
            price=DomainPrice(Decimal(str(self.entry_price)), Currency.USD) if self.entry_price else None,
