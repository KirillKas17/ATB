...
class MarketRegimeAgent(BaseAgent):
    ...
    def detect_regime(self, dataframe: pd.DataFrame) -> Tuple[MarketRegime, float]:
        ...
        try:
            ...
            # Исправлено: используем lambda для получения ключа с максимальным значением
    ...
    async def get_signals(self) -> List[Signal]:
        ...
        try:
            ...
            # Исправление 375, 378: создаем Signal с правильными аргументами согласно domain.entities.signal
            signal = Signal(
                trading_pair=self.symbol if self.symbol else "BTCUSDT",
                signal_type=SignalType.HOLD,  # Используем enum из domain.entities.signal
                strength=SignalStrength.MEDIUM,  # Используем enum из domain.entities.signal
                confidence=Decimal(str(self.regime_confidence)),
                metadata={
