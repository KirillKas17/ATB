...
class NotificationConfig:
    ...
    channels: List[str] = field(default_factory=lambda: ["email", "telegram", "webhook"])  # Каналы уведомлений
    log_dir: str = "logs"  # Директория для логов

    # Email
    smtp_server: str = ""  # SMTP сервер
    smtp_port: int = 587  # SMTP порт
    smtp_username: str = ""  # SMTP логин
    smtp_password: str = ""  # SMTP пароль
    email_from: str = ""  # Email отправителя
    email_to: List[str] = field(default_factory=list)  # Email получателей

    # Telegram
    telegram_token: str = ""  # Telegram токен
...
class NotificationSystem:
    ...
    def __init__(self, config: Optional[Union[Dict[str, Any], NotificationConfig]] = None):
        ...
        Args:
            config: Словарь с параметрами уведомлений или объект NotificationConfig
        """
    ...
    def send(self, subject: str, message: str, level: str = "info") -> None:
        """
        Отправка уведомления.
    ...
    def send_info(self, subject: str, message: str) -> None:
        """
    ...
    def send_warning(self, subject: str, message: str) -> None:
        """
    ...
    def send_error(self, subject: str, message: str) -> None:
        """
    ...
    def __enter__(self) -> "NotificationSystem":
