...
def covariance(x: ArrayLike, y: ArrayLike, window: Optional[int] = None) -> np.ndarray:
    ...
    y = np.asarray(y)
    if window is None:
        return np.array([np.cov(x, y)[0, 1]])
    else:
        result = np.zeros(len(x))
        for i in range(len(x)):
            if i < window:
...
def calculate_performance(
    ...
    # Drawdown duration
    drawdown_duration = int(np.max(np.where(drawdown == max_drawdown)[0]) - np.min(
        np.where(drawdown == max_drawdown)[0]
    ))
...
def calculate_drawdown(returns: ArrayLike) -> Tuple[np.ndarray, float, int]:
    ...
    drawdown = (cumulative_returns - running_max) / running_max
    max_drawdown = np.min(drawdown)
    max_drawdown_duration = int(np.max(np.where(drawdown == max_drawdown)[0]) - np.min(
        np.where(drawdown == max_drawdown)[0]
    ))
