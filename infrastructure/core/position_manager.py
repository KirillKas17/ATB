...
Этот модуль содержит классы для представления и управления торговыми позициями,
включая расчет P&L, ROI, управление рисками и метриками.
"""
...
class Position:
    ...
    def is_profitable(self) -> bool:
        ...
        Returns:
            bool: True если позиция прибыльная
        """
...
class PositionManager:
    ...
    def get_positions_by_symbol(self, symbol: str) -> List[Position]:
        ...
        Args:
            symbol: Торговая пара
        Returns:
            List[Position]: Список позиций по символу
        """
    ...
    def get_total_pnl(self) -> float:
        ...
        Returns:
            float: Общий P&L
        """
        total_pnl = sum(p.pnl or 0 for p in self.positions.values()) + sum(
    ...
    def get_position_correlation(self, symbol1: str, symbol2: str) -> float:
        ...
        correlation_matrix = np.corrcoef(pnl1, pnl2)
    ...
    def _check_position_limits(self, symbol: str, size: float) -> bool:
        ...
        Args:
            ...
            size: Размер позиции
        Returns:
            bool: True если лимиты соблюдены
        """
    ...
    def _check_risk_limits(
        ...
        Args:
            ...
            leverage: Плечо
        Returns:
            bool: True если лимиты риска соблюдены
        """
        ...
        for other_symbol in set(p.symbol for p in self.positions.values()):
            if other_symbol != symbol:
                ...
                if abs(correlation) > self.config["max_correlation"]:
                    logger.warning(
                        f"High correlation {correlation} between {symbol} and "
                        f"{other_symbol}"
                    )
                    return False
        return True
    ...
    def _generate_position_id(self, position: Position) -> str:
        ...
        Args:
            position: Позиция
        Returns:
            str: Уникальный ID позиции
        """
        return (
            f"{position.symbol}_{position.side.value}_"
            f"{int(position.entry_time.timestamp())}"
        )
    ...
    def _check_take_profit(self, position: Position, current_price: float) -> bool:
        ...
        Args:
            ...
            current_price: Текущая цена
        Returns:
            bool: True если тейк-профит сработал
        """
        if position.side == PositionSide.LONG:
    ...
    def _convert_to_float(self, value: Any) -> float:
        ...
        Args:
            value: Значение для преобразования
        Returns:
            float: Числовое представление значения
        """
        if value is None:
            return 0.0
        try:
