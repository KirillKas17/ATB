...
def initialize_strategy_module() -> bool:
    ...
    Вызывается вручную когда необходимо, а не автоматически при импорте.
    Это предотвращает цикличные зависимости и ускоряет импорт.
...
# ВАЖНО: Убираем автоматическую инициализацию при импорте
# для предотвращения цикличных зависимостей и улучшения производительности
# _initialize_globals()  # Закомментировано - теперь вызывается явно
