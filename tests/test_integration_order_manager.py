...
async def test_update_trailing_stop(order_manager: MockOrderManager) -> None:
    ...
    @pytest.mark.asyncio
    async def test_check_break_even(self, order_manager: MockOrderManager) -> None:
    ...
    @pytest.mark.asyncio
    async def test_cancel_order(self, order_manager: MockOrderManager) -> None:
        """Тест отмены ордера"""
        try:
            # Создание тестового ордера
            order = await order_manager.create_entry_order(
    ...
    @pytest.mark.asyncio
    async def test_leverage_calculation(self, order_manager: MockOrderManager) -> None:
        """Тест расчета плеча"""
