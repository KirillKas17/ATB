...
from infrastructure.repositories.trading.trading_repository import InMemoryTradingRepository, PostgresTradingRepository
...
class TestTradingRepository:
    ...
    def sample_order(self: "TestEvolvableMarketMakerAgent") -> Any:
        return Order(
            id=OrderId(uuid4()),
            trading_pair=TradingPair(Symbol("BTC/USDT"), Currency.BTC, Currency.USDT),
            side=OrderSide.BUY,
...
class TestRiskRepository:
    ...
    def sample_risk_manager(self: "TestEvolvableMarketMakerAgent") -> Any:
        risk_profile = RiskProfile(name="Test Profile", risk_level=RiskLevel.MEDIUM, max_drawdown=Decimal("0.20"))
...
class TestMarketRepository:
    ...
    async def test_get_market_data_by_time_range(self, repository, sample_market_data) -> None:
        ...
        data = await repository.get_market_data_by_time_range(sample_market_data.symbol, start_time, end_time)
        assert len(data) == 1
...
class TestPostgresRepositories:
    ...
    @pytest.mark.asyncio
    async def test_postgres_trading_repository_initialization(self: "TestPostgresRepositories") -> None:
    ...
    @pytest.mark.asyncio
    async def test_postgres_strategy_repository_initialization(self: "TestPostgresRepositories") -> None:
    ...
    @pytest.mark.asyncio
    async def test_postgres_ml_repository_initialization(self: "TestPostgresRepositories") -> None:
    ...
    @pytest.mark.asyncio
    async def test_postgres_portfolio_repository_initialization(self: "TestPostgresRepositories") -> None:
    ...
    @pytest.mark.asyncio
    async def test_postgres_risk_repository_initialization(self: "TestPostgresRepositories") -> None:
    ...
    @pytest.mark.asyncio
    async def test_postgres_market_repository_initialization(self: "TestPostgresRepositories") -> None:
