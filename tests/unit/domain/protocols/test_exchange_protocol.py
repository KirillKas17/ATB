...
from domain.protocols.exchange_protocol import ExchangeProtocol, ConnectionStatus
...
from domain.exceptions import ExchangeError, InsufficientFundsError, InvalidOrderError, NetworkError, TimeoutError
...
class TestExchangeProtocolIntegration:
    ...
    async def test_exchange_protocol_workflow(self, mock_exchange_protocol, sample_order):
        ...
        # Создание ордера
        order_response = {"id": order_id, "status": "PENDING", "filled": 0.0, "remaining": 1.0}
        ...
        order_info = {"id": order_id, "status": "FILLED", "filled": 1.0, "remaining": 0.0, "price": 50000.0}
    ...
    async def test_concurrent_operations(self, mock_exchange_protocol):
        ...
        mock_exchange_protocol.create_order = AsyncMock(
            side_effect=[{"id": str(uuid4()), "status": "PENDING"} for _ in range(3)]
        )
