...
def test_registry_register_strategy() -> None:
    ...
    registered_id = registry.register_strategy(
        strategy=strategy, name="Test Strategy", tags=["trend", "test"], priority=1
    )
...
def test_registry_search_strategies() -> None:
    ...
    for i in range(3):
        ...
        registry.register_strategy(strategy=strategy, name=f"Strategy {i}", tags=["trend", f"test_{i}"], priority=i)
...
def test_registry_update_metrics() -> None:
    ...
    # Обновляем метрики
    registry.update_strategy_metrics(strategy_id=strategy_id, execution_count=10, success_count=7)
