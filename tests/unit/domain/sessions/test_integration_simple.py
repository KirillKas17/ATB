...
class TestSessionServiceIntegration:
    ...
    def test_factory_and_basic_methods(self: "TestSessionServiceIntegration") -> None:
    ...
    def test_get_current_session_context(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
    ...
    def test_analyze_session_influence(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
    ...
    def test_predict_session_behavior(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
    ...
    def test_get_session_recommendations(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        from domain.type_definitions.session_types import SessionType
        recs = service.get_session_recommendations('BTCUSDT', SessionType.ASIAN)
        assert isinstance(recs, list)
    def test_get_session_statistics(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        from domain.type_definitions.session_types import SessionType
        stats = service.get_session_statistics(SessionType.ASIAN)
        assert isinstance(stats, dict)
    def test_is_transition_period(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        result = service.is_transition_period()
        assert isinstance(result, bool)
    def test_get_active_transitions(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        transitions = service.get_active_transitions()
        assert isinstance(transitions, list)
    def test_update_session_profile(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        from domain.type_definitions.session_types import SessionType
        ok = service.update_session_profile(SessionType.ASIAN, {'foo': 'bar'})
        assert isinstance(ok, bool)
    def test_get_session_overlap(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        from domain.type_definitions.session_types import SessionType
        overlap = service.get_session_overlap(SessionType.ASIAN, SessionType.LONDON)
        assert isinstance(overlap, float)
    def test_get_session_phase(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        from domain.type_definitions.session_types import SessionType
        phase = service.get_session_phase(SessionType.ASIAN)
        assert phase is None or isinstance(phase, str)
    def test_get_next_session_change(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        next_change = service.get_next_session_change()
        assert next_change is None or isinstance(next_change, dict)
    def test_clear_cache(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
        service.clear_cache()  # smoke-test, не должно падать
    def test_get_session_health_check(self: "TestSessionServiceIntegration") -> None:
        service = get_session_service()
