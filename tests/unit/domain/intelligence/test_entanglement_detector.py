...
class TestEntanglementDetector:
    ...
    def detector(self, sample_config: EntanglementConfig) -> EntanglementDetector:
        """Тестовый детектор."""
        return EntanglementDetector(config=sample_config, enable_advanced_metrics=True, enable_cross_correlation=True)
    ...
    def sample_exchange_data(self, sample_order_book: OrderBookSnapshot) -> Dict[str, Any]:
        ...
        return {
            "binance": {"order_book": sample_order_book, "timestamp": Timestamp.now(), "symbol": "BTC/USDT"},
            "coinbase": {"order_book": sample_order_book, "timestamp": Timestamp.now(), "symbol": "BTC/USD"},
            "kraken": {"order_book": sample_order_book, "timestamp": Timestamp.now(), "symbol": "BTC/USD"},
        }
    ...
    def test_initialization_custom_config(self, sample_config: EntanglementConfig) -> None:
        ...
        detector = EntanglementDetector(
            config=sample_config, enable_advanced_metrics=False, enable_cross_correlation=False
        )
    ...
    def test_detect_entanglement_valid_data(
        self, detector: EntanglementDetector, sample_exchange_data: Dict[str, Any]
    ) -> None:
    ...
    def test_detect_entanglement_insufficient_exchanges(self, detector: EntanglementDetector) -> None:
        ...
        exchange_data = {
            "binance": {"order_book": Mock(spec=OrderBookSnapshot), "timestamp": Timestamp.now(), "symbol": "BTC/USDT"}
        }
    ...
    def test_detect_entanglement_custom_parameters(
        self, detector: EntanglementDetector, sample_exchange_data: Dict[str, Any]
    ) -> None:
        """Тест детекции запутанности с кастомными параметрами."""
        result = detector.detect_entanglement(
            "BTC/USDT", sample_exchange_data, max_lag_ms=5.0, correlation_threshold=0.8
        )

        assert isinstance(result, EntanglementResult)
        assert result.symbol == "BTC/USDT"

    def test_extract_order_books_valid_data(
        self, detector: EntanglementDetector, sample_exchange_data: Dict[str, Any]
    ) -> None:
    ...
    def test_extract_order_books_invalid_data(self, detector: EntanglementDetector) -> None:
        ...
        invalid_data = {"binance": {"invalid": "data"}, "coinbase": {"order_book": "not_an_order_book"}}
    ...
    def test_compute_correlation_matrix(
        self, detector: EntanglementDetector, sample_order_book: OrderBookSnapshot
    ) -> None:
        """Тест вычисления корреляционной матрицы."""
        order_books = {"binance": sample_order_book, "coinbase": sample_order_book, "kraken": sample_order_book}
    ...
    def test_find_best_entanglement_pair(self, detector: EntanglementDetector) -> None:
        ...
        correlation_matrix = {("binance", "coinbase"): 0.8, ("binance", "kraken"): 0.6, ("coinbase", "kraken"): 0.9}
        lag_matrix = {("binance", "coinbase"): 5.0, ("binance", "kraken"): 15.0, ("coinbase", "kraken"): 3.0}

        best_pair, correlation, lag = detector._find_best_entanglement_pair(correlation_matrix, lag_matrix, 10.0)
    ...
    def test_create_analysis_metadata(
        self, detector: EntanglementDetector, sample_order_book: OrderBookSnapshot
    ) -> None:
        """Тест создания метаданных анализа."""
        order_books = {"binance": sample_order_book, "coinbase": sample_order_book}
        correlation_matrix = {("binance", "coinbase"): 0.8}
        lag_matrix = {("binance", "coinbase"): 5.0}
        start_time = 1234567890.0

        metadata = detector._create_analysis_metadata(order_books, correlation_matrix, lag_matrix, start_time)
    ...
    def test_entanglement_result_structure(
        self, detector: EntanglementDetector, sample_exchange_data: Dict[str, Any]
    ) -> None:
    ...
    def test_performance_with_large_data(self, detector: EntanglementDetector) -> None:
        ...
        exchange_data = {
            f"exchange_{i}": {"order_book": large_order_book, "timestamp": Timestamp.now(), "symbol": "BTC/USDT"}
            for i in range(5)
        }
    ...
    def test_different_entanglement_types(self, detector: EntanglementDetector) -> None:
        ...
        high_corr_order_book.bids = [
            (Price(50000 + i, Currency("USD")), Volume(1.0, Currency("BTC"))) for i in range(10)
        ]
        high_corr_order_book.asks = [
            (Price(50100 + i, Currency("USD")), Volume(1.0, Currency("BTC"))) for i in range(10)
        ]
        high_corr_order_book.timestamp = Timestamp.now()

        low_corr_order_book = Mock(spec=OrderBookSnapshot)
        low_corr_order_book.bids = [
            (Price(50000 + i * 10, Currency("USD")), Volume(1.0, Currency("BTC"))) for i in range(10)
        ]
        low_corr_order_book.asks = [
            (Price(50100 + i * 10, Currency("USD")), Volume(1.0, Currency("BTC"))) for i in range(10)
        ]
