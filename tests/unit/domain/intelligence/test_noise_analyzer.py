...
class TestNoiseAnalyzer:
    ...
    def analyzer(self, sample_config: NoiseAnalysisConfig) -> NoiseAnalyzer:
        """Тестовый анализатор."""
        return NoiseAnalyzer(config=sample_config, enable_advanced_metrics=True, enable_frequency_analysis=True)
    ...
    def test_initialization_custom_config(self, sample_config: NoiseAnalysisConfig) -> None:
        ...
        analyzer = NoiseAnalyzer(config=sample_config, enable_advanced_metrics=False, enable_frequency_analysis=False)

        assert analyzer.config == sample_config
        assert analyzer.enable_advanced_metrics is False
        assert analyzer.enable_frequency_analysis is False

    def test_extract_time_series_valid_data(
        self, analyzer: NoiseAnalyzer, sample_order_book: OrderBookSnapshot
    ) -> None:
    ...
    def test_update_history_window_size_limit(
        self, analyzer: NoiseAnalyzer, sample_order_book: OrderBookSnapshot
    ) -> None:
    ...
    def test_compute_fractal_dimension_insufficient_history(
        self, analyzer: NoiseAnalyzer, sample_order_book: OrderBookSnapshot
    ) -> None:
    ...
    def test_compute_entropy_insufficient_history(
        self, analyzer: NoiseAnalyzer, sample_order_book: OrderBookSnapshot
    ) -> None:
    ...
    def test_analyze_noise_insufficient_data(
        self, analyzer: NoiseAnalyzer, sample_order_book: OrderBookSnapshot
    ) -> None:
