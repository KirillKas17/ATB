...
from domain.market.market_data import OHLCV, MarketData, _default_metadata
...
class TestMarketData:
    ...
    def bullish_market_data(self) -> MarketData:
        ...
        return MarketData(symbol="BTC/USD", open=50000.0, high=51000.0, low=49000.0, close=50500.0, volume=1000.0)

    @pytest.fixture
    def bearish_market_data(self) -> MarketData:
        """Медвежьи рыночные данные."""
        return MarketData(symbol="BTC/USD", open=50500.0, high=51000.0, low=49000.0, close=50000.0, volume=1000.0)

    @pytest.fixture
    def doji_market_data(self) -> MarketData:
        """Доджи рыночные данные."""
        return MarketData(symbol="BTC/USD", open=50000.0, high=50100.0, low=49900.0, close=50000.0, volume=1000.0)
