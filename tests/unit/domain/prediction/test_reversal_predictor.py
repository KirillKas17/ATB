...
class TestReversalPredictor:
    ...
    def test_predict_reversal_success(
        ...
        with patch.object(predictor, '_analyze_divergences') as mock_divergences, \
             patch.object(predictor, '_analyze_candlestick_patterns') as mock_patterns, \
             patch.object(predictor, '_analyze_momentum') as mock_momentum, \
             patch.object(predictor, '_analyze_mean_reversion') as mock_mean_rev, \
             patch.object(predictor, '_determine_reversal_direction') as mock_direction, \
             patch.object(predictor, '_calculate_reversal_level') as mock_level, \
             patch.object(predictor, '_calculate_confidence') as mock_confidence, \
             patch.object(predictor, '_calculate_signal_strength') as mock_strength:
    ...
    def test_predict_reversal_insufficient_data(
        self, 
        predictor: ReversalPredictor
    ) -> None:
    ...
    def test_predict_reversal_exception_handling(
        self, 
        predictor: ReversalPredictor
    ) -> None:
    ...
    def test_analyze_divergences(self, predictor: ReversalPredictor, sample_ohlcv_data: pd.DataFrame) -> None:
        ...
        with patch.object(predictor, '_detect_rsi_divergences') as mock_rsi, \
             patch.object(predictor, '_detect_macd_divergences') as mock_macd:
    ...
    def test_detect_rsi_divergences_bearish(
        ...
        with patch.object(predictor, '_calculate_rsi') as mock_rsi, \
             patch.object(predictor, '_find_peaks') as mock_peaks:
    ...
    def test_detect_rsi_divergences_bullish(
        self, 
        predictor: ReversalPredictor
    ) -> None:
    ...
    def test_detect_macd_divergences(self, predictor: ReversalPredictor) -> None:
        ...
        with patch.object(predictor, '_calculate_macd') as mock_macd, \
             patch.object(predictor, '_find_peaks') as mock_peaks:
    ...
    def test_analyze_candlestick_patterns(self, predictor: ReversalPredictor) -> None:
        ...
        with patch.object(predictor, '_is_doji') as mock_doji, \
             patch.object(predictor, '_is_hammer') as mock_hammer, \
             patch.object(predictor, '_is_shooting_star') as mock_shooting:
    ...
    def test_analyze_momentum_insufficient_data(self, predictor: ReversalPredictor) -> None:
        ...
        small_data = pd.DataFrame({
            'close': [100, 101, 102, 103, 104]  # Меньше 20 точек
        })
    ...
    def test_determine_reversal_direction_bullish(
        ...
        divergence_signals = [
            Mock(spec=DivergenceSignal, type=DivergenceType.BULLISH_REGULAR)
        ]
        candlestick_patterns = [
            Mock(spec=CandlestickPattern, name="hammer")
        ]
    ...
    def test_determine_reversal_direction_bearish(
        ...
        divergence_signals = [
            Mock(spec=DivergenceSignal, type=DivergenceType.BEARISH_REGULAR)
        ]
        candlestick_patterns = [
            Mock(spec=CandlestickPattern, name="shooting_star")
        ]
    ...
    def test_calculate_reversal_level_bullish(
        ...
        result = predictor._calculate_reversal_level(
            ReversalDirection.BULLISH, sample_ohlcv_data, [], low_pivots, None
        )
    ...
    def test_calculate_reversal_level_neutral(
        ...
        result = predictor._calculate_reversal_level(
            ReversalDirection.NEUTRAL, sample_ohlcv_data, [], [], None
        )
    ...
    def test_calculate_confidence(
        ...
        divergence_signals = [
            Mock(spec=DivergenceSignal, confidence=ConfidenceScore(0.8))
        ]
        candlestick_patterns = [
            Mock(spec=CandlestickPattern, confirmation_level=0.7)
        ]
    ...
    def test_calculate_signal_strength(
        self, 
        predictor: ReversalPredictor
    ) -> None:
        ...
        divergence_signals = [
            Mock(spec=DivergenceSignal, strength=0.7)
        ]
        candlestick_patterns = [
            Mock(spec=CandlestickPattern, strength=0.6)
        ]
    ...
    def test_is_doji_true(self, predictor: ReversalPredictor) -> None:
        ...
        candle = pd.Series({
            ...
            'high': 101.0,
            'low': 99.0,
            'close': 100.1  # Очень маленькое тело
        })
    ...
    def test_is_hammer_false(self, predictor: ReversalPredictor) -> None:
        ...
        candle = pd.Series({
            'open': 100.0,
            'high': 110.0,  # Длинная верхняя тень
            'low': 99.0,
            'close': 100.5
        })
