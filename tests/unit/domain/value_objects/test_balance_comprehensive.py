...
class TestBalanceCreation:
    ...
    def test_balance_creation_valid(self, mock_post_init):
        ...
        balance = Balance(
            currency=Currency.USD,  # Используем Currency enum для работы total
            free=free_money,
            used=used_money
        )
...
class TestBalanceUtilityMethods:
    ...
    def test_balance_inequality_different_amounts(self):
        ...
        balance1 = Balance(
            currency='USD',
            free=Money(Decimal('1000.00'), Currency.USD),
            used=Money(Decimal('200.00'), Currency.USD)
        )
        
        balance2 = Balance(
            currency='USD',
            free=Money(Decimal('800.00'), Currency.USD),
            used=Money(Decimal('200.00'), Currency.USD)
        )
    ...
    def test_balance_inequality_different_currency(self):
        ...
        balance2 = Balance(
            currency='EUR',
            free=Money(Decimal('1000.00'), Currency.EUR),
            used=Money(Decimal('200.00'), Currency.EUR)
        )
...
class TestBalanceEdgeCases:
    ...
    def test_balance_chain_operations(self):
        ...
        initial_balance = Balance(
            currency='USD',
            free=Money(Decimal('1000.00'), Currency.USD),
            used=Money(Decimal('0'), Currency.USD)
        )
...
class TestBalanceIntegrationWithMocks:
    ...
    def test_balance_factory_pattern(self):
        ...
        def create_empty_balance(currency):
            return Balance(
                currency=currency,
                free=Money(Decimal('0'), currency),
                used=Money(Decimal('0'), currency)
            )
    ...
    def test_balance_builder_pattern(self):
        ...
        balance = (BalanceBuilder()
                  ...
                  .with_free_amount(750.50)
                  .with_used_amount(249.50)
                  .build())
