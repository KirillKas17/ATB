...
class TestTechnicalAnalysisService:
    ...
    def test_analyze_market_data(self, service):
        ...
        market_data = [
            MarketData(
                symbol="BTC/USD", timestamp=datetime.now(), open=100.0, high=105.0, low=95.0, close=102.0, volume=1000.0
            ),
            MarketData(
                symbol="BTC/USD",
...
class TestDefaultTechnicalAnalysisService:
    ...
    def config(self) -> IndicatorConfig:
        ...
        return IndicatorConfig(rsi_period=14, macd_fast=12, macd_slow=26, bb_period=20, use_cache=True)
