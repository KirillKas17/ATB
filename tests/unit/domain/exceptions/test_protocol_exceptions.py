...
class TestProtocolViolationError:
    ...
    def test_protocol_violation_severity(self):
        ...
        critical_error = ProtocolViolationError(
            "REST API", 
        ...
        minor_error = ProtocolViolationError(
            "WebSocket", 
            "heartbeat_missed", 
            "Пропущен heartbeat"
        )
...
class TestExchangeConnectionError:
    ...
    def test_exchange_connection_retryable(self):
        ...
        # Повторяемая ошибка
        retryable_error = ExchangeConnectionError(
            "Binance", "REST", "timeout", retry_after=timedelta(seconds=5)
        )
        
        # Неповторяемая ошибка
        non_retryable_error = ExchangeConnectionError(
            "Coinbase", "REST", "authentication_failed"
        )
...
class TestDataIntegrityError:
    ...
    def test_data_integrity_severity_calculation(self):
        ...
        # Низкая серьезность
        minor_error = DataIntegrityError(
            "order_book", "depth_consistency", 100, 99, threshold=0.05
        )
        
        # Высокая серьезность
        major_error = DataIntegrityError(
            "trade_data", "price_consistency", 100, 50, threshold=0.01
        )
...
class TestAuthenticationError:
    ...
    def test_authentication_severity(self):
        ...
        # Критическая ошибка
        critical_error = AuthenticationError(
            "API_KEY", "key_revoked", "Ключ API отозван"
        )
        
        # Некритическая ошибка
        minor_error = AuthenticationError(
            "JWT_TOKEN", "token_refresh_needed", "Требуется обновление токена"
        )
...
class TestProtocolExceptionIntegration:
    ...
    def test_exception_serialization(self):
        ...
        for exception in exceptions:
            ...
            assert exception_dict["exception_type"] in [
                "ProtocolViolationError", "ExchangeConnectionError", 
                "DataIntegrityError", "RateLimitError", "AuthenticationError"
            ]
    ...
    def test_exception_context_preservation(self):
        ...
        protocol_error = ProtocolViolationError(
            "REST", "validation_error", "Invalid data", context=context
        )
