...
class TestPortfolio:
    ...
    def test_get_margin_ratio_zero_equity(self):
        ...
        portfolio = Portfolio(
            total_equity=Money(Decimal("0"), Currency.USD), used_margin=Money(Decimal("1000"), Currency.USD)
        )
    ...
    def test_margin_ratio_calculation_edge_cases(self):
        ...
        # Нулевой used_margin
        portfolio = Portfolio(
            total_equity=Money(Decimal("10000"), Currency.USD), used_margin=Money(Decimal("0"), Currency.USD)
        )
        assert portfolio.get_margin_ratio() == Decimal("0")

        # Отрицательный equity
        portfolio = Portfolio(
            total_equity=Money(Decimal("-1000"), Currency.USD), used_margin=Money(Decimal("1000"), Currency.USD)
        )
