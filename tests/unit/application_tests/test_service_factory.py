...
from application.services.service_factory import (
    ServiceFactory, DefaultServiceFactory, ServiceFactoryRegistry,
    get_service_factory
)
from application.protocols.service_protocols import (
    MarketService, MLService, TradingService, StrategyService,
    PortfolioService, RiskService, CacheService, NotificationService
)
class TestServiceFactory:
    """Тесты для абстрактной фабрики сервисов."""
    def test_service_factory_is_abstract(self) -> None:
...
class TestDefaultServiceFactory:
    ...
    def test_create_market_service(self) -> None:
    ...
    def test_create_ml_service(self) -> None:
    ...
    def test_create_trading_service(self) -> None:
    ...
    def test_create_strategy_service(self) -> None:
    ...
    def test_create_portfolio_service(self) -> None:
    ...
    def test_create_risk_service(self) -> None:
    ...
    def test_get_service_instance(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_initialize_all_services(self) -> None:
...
class TestServiceFactoryRegistry:
    ...
    def test_create_default_factory(self) -> None:
        ...
        assert created is mock_factory_instance
        self.mock_factory_class.assert_called_once_with(config)
    def test_create_factory_not_found(self) -> None:
        """Тест создания несуществующей фабрики."""
        created = self.registry.create_factory("non_existent")
        assert created is None
    def test_create_default_factory_not_set(self) -> None:
...
class TestGetServiceFactory:
    ...
    def test_get_service_factory_default(self) -> None:
    ...
    def test_get_service_factory_named(self) -> None:
        """Тест получения именованной фабрики."""
        config = {'test': 'config'}
        factory = get_service_factory("default", config)
        assert isinstance(factory, DefaultServiceFactory)
    def test_get_service_factory_fallback(self) -> None:
...
class TestServiceFactoryIntegration:
    ...
    async def test_full_service_lifecycle(self) -> None:
        ...
        assert all([
            market_service, ml_service, trading_service, strategy_service,
            portfolio_service, risk_service, cache_service, notification_service
        ])
        # Инициализируем все сервисы
        await self.factory.initialize_all_services()
        # Проверяем что все сервисы инициализированы
        ...
        # Проверяем что сервисы очищены
        assert len(self.factory.get_all_services()) == 0
    def test_service_configuration_inheritance(self) -> None:
    ...
    def test_service_singleton_behavior(self) -> None:
