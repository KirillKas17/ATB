...
class TestCombinedPredictor:
    ...
    async def test_combine_predictions(self, predictor: CombinedPredictor) -> None:
        ...
        combined = predictor._combine_predictions(
            "BTC/USD", pattern_prediction, session_signals, aggregated_session_signal, timestamp
        )
    ...
    async def test_get_prediction_with_session_context(self, predictor: CombinedPredictor) -> None:
        ...
        result = predictor.get_prediction_with_session_context(symbol, pattern_prediction, market_context)
    ...
    def test_determine_consensus_direction_neutral(
        self, predictor: CombinedPredictor, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_get_technical_prediction(self, predictor: CombinedPredictor, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def _get_technical_prediction(market_data: dict[str, Any]) -> dict[str, Any]:
            return {"direction": "bullish", "confidence": 0.7, "strength": 0.8}
    ...
    def test_get_pattern_prediction(self, predictor: CombinedPredictor, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def _get_pattern_prediction(market_data: dict[str, Any]) -> dict[str, Any]:
            return {"patterns": ["double_top", "support_level"], "confidence": 0.6, "direction": "bearish"}
    ...
    def test_get_ml_prediction(self, predictor: CombinedPredictor, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def _get_ml_prediction(market_data: dict[str, Any]) -> dict[str, Any]:
            return {"prediction": 0.75, "confidence": 0.8, "model": "transformer"}
...
class TestPatternPredictor:
    ...
    def test_predict_with_custom_features(self, predictor: PatternPredictor, mock_pattern_memory: Mock) -> None:
        ...
        result = predictor.predict_with_custom_features(symbol, PatternType.DOUBLE_TOP, features, market_context)
...
class TestReversalController:
    ...
    @pytest.mark.asyncio
    async def test_analyze_signal_strength(
        self, controller: ReversalController, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_calculate_reversal_probability(
        self, controller: ReversalController, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_determine_overall_direction(
        self, controller: ReversalController, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_determine_overall_direction_neutral(
        self, controller: ReversalController, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Тест определения нейтрального общего направления."""
        signals = [{"direction": "bullish", "strength": 0.5}, {"direction": "bearish", "strength": 0.5}]
    ...
    @pytest.mark.asyncio
    async def test_validate_signal_data_invalid(
        self, controller: ReversalController, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_validate_signal_data_empty(
        self, controller: ReversalController, monkeypatch: pytest.MonkeyPatch
    ) -> None:
