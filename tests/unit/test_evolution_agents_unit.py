...
class TestEvolvableMetaController:
    ...
    def test_extract_features(self, agent, sample_data) -> None:
        """Тест экстракции признаков"""
        features = agent._extract_features(sample_data["market_data"], sample_data["strategy_signals"], sample_data["risk_metrics"])
        assert isinstance(features, list)
    ...
    async def test_optimize_decision(self, agent, sample_data) -> None:
        ...
        result = await agent.optimize_decision(sample_data["market_data"], sample_data["strategy_signals"], sample_data["risk_metrics"])
        assert isinstance(result, dict)
