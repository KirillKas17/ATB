...
from domain.type_definitions.external_service_types import MLModelConfig, MLPredictionRequest
...
class TestMLServiceConfig:
    ...
    def test_custom_config(self: "TestMLServiceConfig") -> None:
        ...
        config = MLServiceConfig(
            service_url="http://custom:8002", models_dir="/custom/models", max_models=50, enable_caching=False
        )
...
class TestProductionMLService:
    ...
    async def test_predict(self, ml_service, sample_model_config, sample_training_data) -> None:
        ...
        request = MLPredictionRequest(
            model_id=model_id, features={"feature_0": 0.5, "feature_1": 0.3}, confidence_threshold=0.8
        )
    ...
    async def test_evaluate_model(self, ml_service, sample_model_config, sample_training_data) -> None:
        ...
        test_data = {"features": np.random.normal(0, 1, (50, 10)), "target": np.random.normal(0, 1, 50)}
...
class TestMLServiceAdapter:
    ...
    async def test_train_model(self, adapter) -> None:
        ...
        with patch("requests.post") as mock_post:
            ...
            model = await adapter.train_model(
                ModelType.RANDOM_FOREST, {"features": [[1, 2, 3]], "target": [1]}, {"n_estimators": 100}
            )
    ...
    async def test_predict(self, adapter) -> None:
        ...
        with patch("requests.post") as mock_post:
            mock_post.return_value.json.return_value = {"prediction": 1.5, "confidence": 0.9}
            mock_post.return_value.status_code = 200
...
class TestLocalMLService:
    ...
    async def test_predict(self, local_service) -> None:
        ...
        # Сначала обучаем модель
        model = await local_service.train_model(
            ModelType.RANDOM_FOREST, {"features": [[1, 2, 3], [4, 5, 6]], "target": [1, 2]}, {"n_estimators": 100}
        )
        # Делаем предсказание
    ...
    async def test_evaluate_model(self, local_service) -> None:
        ...
        # Оцениваем модель
        metrics = await local_service.evaluate_model(model.id, {"features": [[1, 2, 3]], "target": [1]})
        assert isinstance(metrics, dict)
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_invalid_model_type(self: "TestErrorHandling") -> None:
    ...
    @pytest.mark.asyncio
    async def test_empty_training_data(self: "TestErrorHandling") -> None:
        """Тест обработки пустых данных обучения."""
        service = ProductionMLService()
        config = MLModelConfig(model_type=ModelType.RANDOM_FOREST, name="test", version="1.0.0", hyperparameters={})
        with pytest.raises(ValueError):
            await service.train_model(config, {})

    @pytest.mark.asyncio
    async def test_invalid_prediction_request(self: "TestErrorHandling") -> None:
    ...
    @pytest.mark.asyncio
    async def test_model_not_found(self: "TestErrorHandling") -> None:
        """Тест обработки отсутствующей модели."""
        service = ProductionMLService()
        request = MLPredictionRequest(model_id="non_existent", features={"feature_1": 0.5}, confidence_threshold=0.8)
        with pytest.raises(ValueError):
