    ...
    def mock_exchange(self, exchange_config: ExchangeConfig) -> Mock:
        ...
        exchange.get_orderbook = AsyncMock(
            return_value=OrderBook(
                ...
                bids=[
                    OrderBookEntry(
                        Price(Decimal("49999.0"), Currency.USDT, Currency.USDT),
                        Volume(Decimal("1.0"), Currency.BTC),
                        TimestampValue(datetime.utcnow()),
                    ),
                    OrderBookEntry(
                        Price(Decimal("49998.0"), Currency.USDT, Currency.USDT),
                        Volume(Decimal("2.0"), Currency.BTC),
                        TimestampValue(datetime.utcnow()),
                    ),
                ],
                asks=[
                    OrderBookEntry(
                        Price(Decimal("50001.0"), Currency.USDT, Currency.USDT),
                        Volume(Decimal("1.5"), Currency.BTC),
                        TimestampValue(datetime.utcnow()),
                    ),
                    OrderBookEntry(
                        Price(Decimal("50002.0"), Currency.USDT, Currency.USDT),
                        Volume(Decimal("2.5"), Currency.BTC),
                        TimestampValue(datetime.utcnow()),
                    ),
                ],
            )
        ...
        exchange.get_balance = AsyncMock(
            return_value=Balance(currency="USDT", available=Decimal("10000.0"), locked=Decimal("0.0"))
        )
        exchange.get_balances = AsyncMock(
            return_value={
                "USDT": Balance(currency="USDT", available=Decimal("10000.0"), locked=Decimal("0.0")),
                "BTC": Balance(currency="BTC", available=Decimal("1.0"), locked=Decimal("0.0")),
            }
        )
...
class TestConnectionProtocol:
    ...
    @pytest.mark.asyncio
    async def test_connection_state_transitions(self: "TestConnectionProtocol") -> None:
