...
from domain.type_definitions.external_service_types import ExchangeType, ExchangeCredentials, ConnectionConfig
...
class TestExchangeServiceFactory:
    ...
    def sample_connection_config(self: "TestEvolvableMarketMakerAgent") -> Any:
        ...
        return ConnectionConfig(rate_limit=100, rate_limit_window=60, timeout=30.0, retry_attempts=3)
    ...
    def test_create_exchange_service_bybit(self, sample_credentials, sample_connection_config) -> None:
        ...
        with patch("infrastructure.external_services.exchanges.bybit_exchange_service.BybitExchangeService"):
            service = ExchangeServiceFactory.create_exchange_service(
                ExchangeType.BYBIT, sample_credentials, sample_connection_config
            )
    ...
    def test_create_exchange_service_binance(self, sample_credentials, sample_connection_config) -> None:
        ...
        with patch("infrastructure.external_services.exchanges.binance_exchange_service.BinanceExchangeService"):
            service = ExchangeServiceFactory.create_exchange_service(
                ExchangeType.BINANCE, sample_credentials, sample_connection_config
            )
    ...
    def test_create_exchange_service_unsupported_type(self, sample_credentials) -> None:
        ...
        with pytest.raises(ValueError, match="Unsupported exchange type"):
            ExchangeServiceFactory.create_exchange_service(
                ExchangeType.UNKNOWN, sample_credentials  # Предполагаем, что такой тип существует
            )
    ...
    def test_create_exchange_service_default_connection_config(self, sample_credentials) -> None:
        ...
        with patch("infrastructure.external_services.exchanges.bybit_exchange_service.BybitExchangeService"):
            service = ExchangeServiceFactory.create_exchange_service(ExchangeType.BYBIT, sample_credentials)
            assert isinstance(service, BybitExchangeService)
    ...
    def test_create_bybit_service(self: "TestExchangeServiceFactory") -> None:
        ...
        with patch("infrastructure.external_services.exchanges.bybit_exchange_service.BybitExchangeService"):
            service = ExchangeServiceFactory.create_bybit_service(
                api_key="bybit_key", api_secret="bybit_secret", testnet=True
            )
            assert isinstance(service, BybitExchangeService)
    ...
    def test_create_binance_service(self: "TestExchangeServiceFactory") -> None:
        ...
        with patch("infrastructure.external_services.exchanges.binance_exchange_service.BinanceExchangeService"):
            service = ExchangeServiceFactory.create_binance_service(
                api_key="binance_key", api_secret="binance_secret", testnet=True
            )
            assert isinstance(service, BinanceExchangeService)
    ...
    def test_factory_methods_consistency(self: "TestExchangeServiceFactory") -> None:
        ...
        credentials = ExchangeCredentials(api_key="test_key", api_secret="test_secret", testnet=True)
        with patch("infrastructure.external_services.exchanges.bybit_exchange_service.BybitExchangeService"):
            service1 = ExchangeServiceFactory.create_exchange_service(ExchangeType.BYBIT, credentials)
