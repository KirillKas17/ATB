...
from domain.protocols.strategy_protocol import (
    StrategyProtocol, StrategyState, StrategyConfig, StrategyMetrics,
    StrategySignal, StrategyExecutionResult, StrategyError,
...
class TestStrategyProtocol:
    ...
    def mock_strategy(self, strategy_config: StrategyConfig) -> Mock:
        ...
        strategy.analyze_market = AsyncMock(return_value=StrategySignal(
        ...
        strategy.generate_signal = AsyncMock(return_value=StrategySignal(
            timestamp=datetime.utcnow(),
            signal_type="buy",
            confidence=0.8,
            price=Price(Decimal("50000.0"), Currency("USDT")),
        ...
        strategy.execute_signal = AsyncMock(return_value=StrategyExecutionResult(
            success=True,
            order_id="order_123",
