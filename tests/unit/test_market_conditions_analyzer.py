...
class TestMarketConditionsAnalyzer:
    ...
    def mock_technical_analysis_service(self: "TestEvolvableMarketMakerAgent") -> Any:
        ...
        service = Mock()
        service.perform_complete_analysis.return_value = {"indicators": {"rsi": 50.0, "macd": 0.0}, "signals": []}
    ...
    def analyzer(self, mock_market_repository, mock_technical_analysis_service) -> Any:
        ...
        config = MarketConditionsConfig(
            short_window=5, medium_window=10, long_window=20, volatility_window=5, trend_window=10
        )
    ...
    async def test_calculate_market_score_with_valid_data(
        ...
        mock_market_repository.get_market_data.return_value = [
            self._create_market_data_entity(row, i) for i, row in sample_market_data.iterrows()
        ]

        # Выполняем анализ
        result = await analyzer.calculate_market_score(symbol="BTC/USDT", timeframe="1h", lookback_periods=50)
    ...
    def test_calculate_confidence(self, analyzer) -> None:
        ...
        result = analyzer._calculate_confidence(
            volatility_analysis, trend_analysis, volume_analysis, momentum_analysis, regime_analysis
        )
