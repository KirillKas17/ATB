...
class TestFeatureEngineer:
    ...
    def test_generate_features_success(self, engineer: FeatureEngineer, sample_market_data: pd.DataFrame):
        ...
        tech_indicators = [col for col in result.columns if any(
            indicator in col.lower() for indicator in ['ema', 'rsi', 'macd', 'bb']
        )]
    ...
    def test_scale_features(self, engineer: FeatureEngineer, sample_market_data: pd.DataFrame):
        ...
        if len(numeric_cols) > 0:
            ...
            assert all(abs(std - 1) < 0.1 for std in stds if not np.isnan(std)), "Стандартное отклонение должно быть близко к 1"
...
class TestFeatureImportanceAnalyzer:
    ...
    def test_calculate_importance_correlation(self, analyzer: FeatureImportanceAnalyzer,
                                           sample_features_and_target: tuple[pd.DataFrame, pd.Series]):
    ...
    def test_rank_features(self, analyzer: FeatureImportanceAnalyzer,
                          sample_features_and_target: tuple[pd.DataFrame, pd.Series]):
...
class TestMarketRegimeClassifier:
    ...
    def test_classify_regime_sideways(self, classifier: MarketRegimeClassifier,
                                    market_data_sideways: pd.DataFrame):
    ...
    def test_get_regime_features(self, classifier: MarketRegimeClassifier,
                               market_data_trending_up: pd.DataFrame):
