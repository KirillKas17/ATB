...
class TestSecurityManager:
    ...
    async def test_authenticate_and_authorize(self, security_manager: SecurityManager) -> None:
        ...
        await auth_manager.register_user(
            "test_user", "test_password", "test@example.com", {Permission.READ, Permission.WRITE}
        )

        # Тестируем аутентификацию и авторизацию
        context = await security_manager.authenticate_and_authorize(
            "test_user", "test_password", "127.0.0.1", "test_agent", "orders", Permission.READ
        )
...
class TestAuthenticationManager:
    ...
    async def test_authenticate_user(self, auth_manager: AuthenticationManager) -> None:
        ...
        result = await auth_manager.authenticate_user("test_user", "test_password", "127.0.0.1", "test_agent")
    ...
    async def test_authenticate_with_invalid_credentials(self, auth_manager: AuthenticationManager) -> None:
        ...
        result = await auth_manager.authenticate_user("invalid_user", "invalid_password", "127.0.0.1", "test_agent")
...
class TestAuditManager:
    ...
    async def test_log_event(self, audit_manager: AuditManager) -> None:
        ...
        await audit_manager.log_event(AuditEvent.LOGIN, context, "orders", "read", "success")
    ...
    async def test_log_security_violation(self, audit_manager: AuditManager) -> None:
        ...
        await audit_manager.log_security_violation(
            "unauthorized_access", SecurityLevel.HIGH, "Attempted access to restricted resource", context
        )
...
class TestSecurityIntegration:
    ...
    async def test_full_security_workflow(self: "TestSecurityIntegration") -> None:
        ...
        await auth_manager.register_user(
            "test_user", "StrongP@ssw0rd123", "test@example.com", {Permission.READ, Permission.WRITE}
        )

        # 2. Аутентифицируем пользователя
        context = await auth_manager.authenticate_user("test_user", "StrongP@ssw0rd123", "127.0.0.1", "test_agent")
        assert isinstance(context, SecurityContext)
        assert context.user_id == "test_user"
    ...
    @pytest.mark.asyncio
    async def test_concurrent_security_operations(self: "TestSecurityIntegration") -> None:
