...
class TestDataProcessor:
    ...
    def test_remove_outliers(self, data_processor: DataProcessor, sample_market_data: pd.DataFrame) -> None:
        ...
        if (
            hasattr(data_with_outliers, "iloc")
            and hasattr(data_with_outliers.columns, "get_loc")
            and len(data_with_outliers) > 1
        ):
    ...
    async def test_calculate_technical_indicators(
        self, data_processor: DataProcessor, sample_market_data: pd.DataFrame
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_lagged_features(
        self, data_processor: DataProcessor, sample_market_data: pd.DataFrame
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_create_rolling_features(
        self, data_processor: DataProcessor, sample_market_data: pd.DataFrame
    ) -> None:
