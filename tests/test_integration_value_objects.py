...
class TestSignal:
    ...
    def test_signal_factory_methods(self: "TestSignal") -> None:
        ...
        buy_signal = Signal.create_buy_signal(
            timestamp=timestamp, strength=SignalStrength.STRONG, price=Decimal("50000")
        )
        assert buy_signal.is_buy_signal()
        # Sell сигнал
        sell_signal = Signal.create_sell_signal(timestamp=timestamp, strength=SignalStrength.MODERATE)
...
class TestValueObjectFactory:
    ...
    def test_factory_signal_creation(self: "TestValueObjectFactory") -> None:
        ...
        timestamp = Timestamp.now()
        signal = factory.create_signal(SignalType.BUY, timestamp, SignalStrength.STRONG)
...
class TestValueObjectTypes:
    ...
    def test_precision_config(self: "TestValueObjectTypes") -> None:
        ...
        config = PrecisionConfig(mode=PrecisionMode.TRADING, decimal_places=8, rounding_mode="ROUND_HALF_UP")
