    ...
    def time_function(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            self.timing_data[func_name].append(
                {"time": execution_time, "args_count": len(args), "kwargs_count": len(kwargs), "timestamp": time.time()}
            )
            ...
            if times:
                analysis[func_name] = {
                    ...
                    "threshold_violations": sum(
                        1
                        for t in times
                        if self.thresholds.get(func_name) and t > self.thresholds.get(func_name, float("inf"))
                    ),
                }
...
class TestFunctionTiming:
    ...
    def test_pandas_function_timing(self, timer: FunctionTimer, test_data: pd.DataFrame):
        ...
        for func_name, stats in sorted(analysis.items(), key=lambda x: x[1]["avg_time"], reverse=True):
            ...
            print(
                f"{func_name.split('.')[-1]:<30} {avg_ms:<10.2f} {max_ms:<10.2f} {stats['call_count']:<8} {violations:<10}"
            )
        ...
        if slow_functions:
            ...
            for func in slow_functions:
                print(
                    f"  {func['function'].split('.')[-1]}: {func['avg_time']*1000:.2f}ms avg, {func['total_time']*1000:.2f}ms total"
                )
    ...
    def test_numpy_function_timing(self, timer: FunctionTimer, test_data: pd.DataFrame):
        ...
        for func_name, stats in sorted(numpy_funcs.items(), key=lambda x: x[1]["avg_time"], reverse=True):
            ...
            print(
                f"{func_name.split('.')[-1]:<25} {avg_us:<10.0f} {max_us:<10.0f} {p95_us:<10.0f} {stats['call_count']:<8}"
            )
    ...
    def test_custom_function_performance(self, timer: FunctionTimer):
        ...
        custom_funcs = {
            ...
            if any(name in k for name in ["efficient", "inefficient", "memory_intensive"])
        }
    ...
    def test_performance_regression_detection(self, timer: FunctionTimer):
        ...
        for func_name, stats in analysis.items():
            ...
            if baseline_time > 0:
                ...
                print(
                    f"{status} {short_name:<20}: {current_time:>6.1f}ms (baseline: {baseline_time:>6.1f}ms, ratio: {ratio:.1f}x)"
                )
    ...
    def test_generate_timing_report(self, timer: FunctionTimer):
        ...
        threshold_violations = [
            (name, stats) for name, stats in analysis.items() if stats.get("threshold_violations", 0) > 0
        ]
        ...
        # Медленные функции
        slow_funcs = [(name, stats) for name, stats in analysis.items() if stats["avg_time"] > 0.01]  # Медленнее 10ms
