            ...
            for position in positions:
                ...
                updated.append({
                    **position,
                    ...
                    'unrealized_pnl': Decimal('1.0')  # Мок PnL
                })
            return updated
        
        # Подготовка данных
        test_orders = [
            {'order_id': f'order_{i}', 'symbol': 'BTCUSDT', 'quantity': Decimal('0.01')}
            for i in range(1000)
        ]
        
        test_positions = [
            {'position_id': f'pos_{i}', 'symbol': 'BTCUSDT', 'size': Decimal('0.1')}
            for i in range(500)
        ]
