...
from infrastructure.repositories.trading.trading_repository import InMemoryTradingRepository
    ...
    async def test_concurrent_event_processing(self, event_bus) -> None:
        ...
        for i in range(50):
            task = asyncio.create_task(
                asyncio.gather(*[asyncio.create_task(event_bus.publish("test_event", {"value": j})) for j in range(10)])
            )
            tasks.append(task)
    ...
    def sample_trades(self) -> List[Trade]:
        ...
        for i in range(1000):
            trade = Trade(
                ...
                side=(
                    domain.entities.trading.OrderSide.BUY if i % 2 == 0 else domain.entities.trading.OrderSide.SELL
                ),  # Исправление: используем правильный тип
...
class TestMemoryUsage:
    ...
    def test_large_dataset_memory_usage(self: "TestMemoryUsage") -> None:
        ...
        for i in range(100000):
            large_dataset.append({"id": i, "data": f"data_{i}", "timestamp": time.time()})
...
class TestConcurrencyPerformance:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_database_operations(self: "TestConcurrencyPerformance") -> None:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_repository_operations(self: "TestConcurrencyPerformance") -> None:
