        ...
        if os.path.exists(self.baseline_file):
            try:
                with open(self.baseline_file, 'r') as f:
                    data = json.load(f)
                    self.baselines = {
                        name: BenchmarkBaseline(**baseline_data)
...
class TestBenchmarkSuite:
    ...
    def test_pandas_operations_benchmarks(
        self,
        benchmark_runner: BenchmarkRunner,
        medium_dataset: pd.DataFrame
    ):
        ...
        operations = {
            ...
            'resample_5min_ohlc': lambda df: df['close'].resample('5min').ohlc() if hasattr(df.index, 'freq') else df['close'].head(100),
    ...
    def test_numpy_operations_benchmarks(
        self,
        benchmark_runner: BenchmarkRunner,
        medium_dataset: pd.DataFrame
    ):
            ...
            for run in range(10):  # Больше прогонов для быстрых numpy операций
                try:
                    result = benchmark_runner.run_benchmark(
                        benchmark_name,
                        operation,
                        data_array.copy()  # Копируем для избежания побочных эффектов
                    )
                    results.append(result)
                except Exception as e:
                    print(f"Error in {op_name}: {e}")
