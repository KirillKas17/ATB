        ...
        def process_market_data():
            ...
            data["rsi"] = 100 - (
                ...
                / (
                    ...
                    .pct_change()
                    .rolling(14)
                    .apply(lambda x: x[x > 0].sum() / abs(x[x < 0].sum()) if abs(x[x < 0].sum()) != 0 else 0)
                )
            )
...
def generate_performance_report(results: List[PerformanceResult]) -> str:
    ...
    if critical_issues:
        ...
        for result in critical_issues:
            report.append(
                f"- **{result.component_name}**: {result.avg_time_ms:.2f}ms (целевое время зависит от операции)"
            )
...
class TestCriticalPerformance:
    ...
    def test_feature_engineering_performance_acceptable(self):
        ...
        assert result.performance_grade in [
            "A",
            "B",
            "C",
        ], f"Производительность генерации признаков неприемлема: {result.performance_grade}"
    ...
    def test_pattern_recognition_performance_acceptable(self):
        ...
        assert result.performance_grade in [
            ...
            "B",
            "C",
        ], f"Производительность распознавания паттернов неприемлема: {result.performance_grade}"
    ...
    def test_cache_operations_performance_fast(self):
        ...
        assert result.performance_grade in [
            "A",
            "B",
        ], f"Производительность кэша неприемлема: {result.performance_grade}"
    ...
    def test_market_data_processing_performance_acceptable(self):
        ...
        assert result.performance_grade in [
            ...
            "B",
            "C",
        ], f"Производительность обработки данных неприемлема: {result.performance_grade}"
    ...
    def test_comprehensive_performance_report(self):
        ...
        ), f"Обнаружены критические проблемы производительности: {[r.component_name for r in critical_issues]}"
