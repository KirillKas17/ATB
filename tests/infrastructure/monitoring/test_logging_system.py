...
class TestStructuredLogger:
    ...
    def test_log_with_context(self: "TestStructuredLogger") -> None:
        ...
        logger = StructuredLogger()
        context = LogContext(
            request_id="test-123", user_id="user-456", session_id="session-789", correlation_id="corr-abc"
        )
    ...
    def test_log_with_metadata(self: "TestStructuredLogger") -> None:
        ...
        logger = StructuredLogger()
        metadata = {"component": "test_component", "operation": "test_operation", "duration": 1.5}
        with patch.object(logger, "_log") as mock_log:
...
class TestSetupLogging:
    ...
    def test_setup_logging_custom_config(self: "TestSetupLogging") -> None:
        ...
        config = {"level": "DEBUG", "handlers": ["console"], "formatters": {"json": {"format": "json"}}}
        with patch("infrastructure.monitoring.logging_system.StructuredLogger") as mock_logger:
    ...
    def test_setup_logging_file_handler(self: "TestSetupLogging") -> None:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            config = {"handlers": ["file"], "file_config": {"path": temp_file.name}}
            try:
                with patch("infrastructure.monitoring.logging_system.StructuredLogger") as mock_logger:
                    setup_logging(config)
                    mock_logger.assert_called()
...
class TestLogContext:
    ...
    def test_log_context_init(self: "TestLogContext") -> None:
        ...
        context = LogContext(
            request_id="req-123", user_id="user-456", session_id="session-789", correlation_id="corr-abc"
        )
    ...
    def test_log_context_from_dict(self: "TestLogContext") -> None:
        ...
        context_dict = {"request_id": "req-123", "user_id": "user-456", "session_id": "session-789"}
...
class TestLogEntry:
    ...
    def test_log_entry_to_dict(self: "TestLogEntry") -> None:
        ...
        metadata = {"test": "data"}
        entry = LogEntry(
            timestamp=datetime.now(), level=LogLevel.INFO, message="Test message", context=context, metadata=metadata
        )
