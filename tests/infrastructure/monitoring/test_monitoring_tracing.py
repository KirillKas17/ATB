...
try:
    ...
    class MockPerformanceMetrics: pass
    ...
    def test_concurrent_tracing(self: "TestPerformanceTracer") -> None:
        ...
        for i in range(10):
            thread = threading.Thread(
                target=create_trace,
                args=(f"perf-trace-{i}", f"operation{i}")
            )
...
class TestTraceFunctions:
    ...
    def test_add_child_span(self, mock_get_tracer) -> None:
        ...
        mock_tracer.add_child_span.assert_called_once_with(
            "test-trace", "child-span", "child_operation", "parent-span"
        )
        assert span == mock_span
class TestTraceSpan:
    """Тесты для TraceSpan."""
