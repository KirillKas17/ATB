...
from domain.evolution.strategy_fitness import TradeResult, StrategyEvaluationResult
...
class TestEvolutionE2E:
    ...
    def test_complete_evolution_workflow(
        self, temp_db_path: str, temp_backup_dir: Path, temp_migration_dir: Path
    ) -> None:
        ...
        cache = EvolutionCache({"cache_size": 1000, "cache_ttl": 600, "cache_strategy": "lru"})
    ...
    def _select_best_candidates(
        self, evaluations: List[StrategyEvaluationResult], count: int
    ...
    def _generate_next_generation(
        self, best_evaluations: List[StrategyEvaluationResult], context: EvolutionContext, size: int
    ...
    def _crossover(
        self, parent1: StrategyCandidate, parent2: StrategyCandidate, context: EvolutionContext
    ) -> StrategyCandidate:
        ...
        return StrategyCandidate(
            ...
            mutation_count=0,
            indicators=parent1.indicators[: len(parent1.indicators) // 2]
            + parent2.indicators[len(parent2.indicators) // 2 :],
    ...
    def _create_generation_backup(
        self,
        backup: EvolutionBackup,
        generation: int,
    ...
    def _verify_data_integrity(
        self, storage: StrategyStorage, cache: EvolutionCache, backup: EvolutionBackup, final_backup_path: Path
    ) -> Any:
    ...
    def _verify_performance_metrics(
        self, storage: StrategyStorage, cache: EvolutionCache, backup: EvolutionBackup
    ) -> Any:
    ...
    def test_scalability_e2e(self, temp_db_path: str, temp_backup_dir: Path) -> None:
        ...
        storage = StrategyStorage(temp_db_path)
        cache = EvolutionCache({"cache_size": 10000, "cache_ttl": 3600, "cache_strategy": "lru"})
        backup = EvolutionBackup(str(temp_backup_dir))
