...
from infrastructure.evolution.exceptions import ConnectionError, QueryError, StorageError, ValidationError
...
class TestStrategyStorage:
    ...
    def test_save_strategy_candidate_success(
        self, storage: StrategyStorage, sample_candidate: StrategyCandidate
    ) -> None:
        ...
        with Session(storage.engine) as session:
            model = session.exec(
                select(StrategyCandidateModel).where(StrategyCandidateModel.id == str(sample_candidate.id))
            ).first()
            assert model is not None
            assert model.name == sample_candidate.name
            assert model.strategy_type == sample_candidate.strategy_type.value

    def test_save_strategy_candidate_update(
        self, storage: StrategyStorage, sample_candidate: StrategyCandidate
    ) -> None:
    ...
    def test_get_strategy_candidate_success(
        self, storage: StrategyStorage, sample_candidate: StrategyCandidate
    ) -> None:
    ...
    def test_get_strategy_candidates_filtered(
        self, storage: StrategyStorage, sample_candidate: StrategyCandidate
    ) -> None:
    ...
    def test_get_strategy_candidates_limit(self, storage: StrategyStorage) -> None:
        ...
        for i in range(5):
            candidate = StrategyCandidate(id=uuid4(), name=f"Strategy {i}", generation=i)
            storage.save_strategy_candidate(candidate)
        candidates = storage.get_strategy_candidates(limit=3)
        assert len(candidates) == 3

    def test_save_evaluation_result_success(
        self, storage: StrategyStorage, sample_evaluation: StrategyEvaluationResult
    ) -> None:
        ...
        with Session(storage.engine) as session:
            model = session.exec(
                select(StrategyEvaluationModel).where(StrategyEvaluationModel.id == str(sample_evaluation.id))
            ).first()
            assert model is not None
    ...
    def test_save_evaluation_result_validation_error(self, storage: StrategyStorage) -> None:
        ...
        invalid_evaluation = StrategyEvaluationResult(
            id=uuid4(), strategy_id=uuid4(), total_trades=-1  # Отрицательное значение - ошибка валидации
        )
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_get_evaluation_result_success(
        self, storage: StrategyStorage, sample_evaluation: StrategyEvaluationResult
    ) -> None:
    ...
    def test_get_evaluation_results_filtered(
        self, storage: StrategyStorage, sample_evaluation: StrategyEvaluationResult
    ) -> None:
    ...
    def test_save_evolution_context_success(self, storage: StrategyStorage, sample_context: EvolutionContext) -> None:
        ...
        with Session(storage.engine) as session:
            model = session.exec(
                select(EvolutionContextModel).where(EvolutionContextModel.id == str(sample_context.id))
            ).first()
            assert model is not None
    ...
    def test_save_evolution_context_validation_error(self, storage: StrategyStorage) -> None:
        ...
        invalid_context = EvolutionContext(
            id=uuid4(), name="", population_size=0  # Пустое имя - ошибка валидации  # Нулевой размер - ошибка валидации
        )
        with pytest.raises(ValidationError) as exc_info:
    ...
    def test_get_evolution_contexts_limit(self, storage: StrategyStorage) -> None:
        ...
        for i in range(5):
            context = EvolutionContext(id=uuid4(), name=f"Context {i}", population_size=50)
    ...
    def test_cleanup_old_data(self, storage: StrategyStorage) -> None:
        ...
        old_candidate = StrategyCandidate(
            id=uuid4(), name="Old Strategy", created_at=datetime.now() - timedelta(days=40)
        )
        storage.save_strategy_candidate(old_candidate)
        old_evaluation = StrategyEvaluationResult(
            id=uuid4(), strategy_id=uuid4(), evaluation_time=datetime.now() - timedelta(days=40)
        )
        storage.save_evaluation_result(old_evaluation)
        # Создать новые данные
        new_candidate = StrategyCandidate(id=uuid4(), name="New Strategy", created_at=datetime.now())
    ...
    def test_import_data(
        self, storage: StrategyStorage, sample_candidate: StrategyCandidate, sample_context: EvolutionContext
    ) -> None:
        """Тест импорта данных."""
