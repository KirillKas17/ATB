...
class TestStrategyModel:
    ...
    def test_strategy_candidate_with_rules(self: "TestStrategyModel") -> None:
        ...
        exit_rule = ExitRule(stop_loss_pct=Decimal("0.02"), take_profit_pct=Decimal("0.04"))
    ...
    def test_strategy_candidate_serialization(self: "TestStrategyModel") -> None:
        ...
        candidate = StrategyCandidate(name="Test Strategy", strategy_type=StrategyType.MEAN_REVERSION)
        # Добавить индикатор
        indicator = IndicatorConfig(name="RSI", indicator_type=IndicatorType.MOMENTUM, parameters={"period": 14})
        candidate.add_indicator(indicator)
        # Сериализация
...
class TestEvolutionContext:
    ...
    def test_evolution_context_serialization(self: "TestEvolutionContext") -> None:
        ...
        context = EvolutionContext(name="Test Context", population_size=30, generations=50)
...
class TestStrategyFitnessEvaluator:
    ...
    def test_evaluate_strategy(self: "TestStrategyFitnessEvaluator") -> None:
        ...
        candidate = StrategyCandidate(name="Test Strategy", strategy_type=StrategyType.TREND_FOLLOWING)
        # Добавить простые правила
        entry_rule = EntryRule(conditions=[{"simple": True}], signal_type=SignalType.BUY)
        candidate.add_entry_rule(entry_rule)
...
class TestStrategySelector:
    ...
    def test_select_by_fitness(self: "TestStrategySelector") -> None:
        ...
        # Отбор топ-3
        selected = selector.select_top_strategies(candidates, evaluations, n=3, selection_method="fitness")
    ...
    def test_multi_criteria_selection(self: "TestStrategySelector") -> None:
        ...
        for candidate in candidates:
            ...
            evaluations.append(evaluation)
        selected = selector.select_top_strategies(candidates, evaluations, n=2, selection_method="multi_criteria")
...
class TestStrategyStorage:
    ...
    def test_save_and_retrieve_candidate(self: "TestStrategyStorage") -> None:
        ...
        # Создать кандидата
        candidate = StrategyCandidate(name="Test Candidate", strategy_type=StrategyType.TREND_FOLLOWING)
        # Сохранить
    ...
    def test_storage_statistics(self: "TestStrategyStorage") -> None:
        ...
        candidate = StrategyCandidate()
        storage.save_strategy_candidate(candidate)
        evaluation = StrategyEvaluationResult(strategy_id=candidate.id, is_approved=True)
        storage.save_evaluation_result(evaluation)
