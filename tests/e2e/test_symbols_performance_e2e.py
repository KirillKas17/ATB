...
from domain.symbols import (
    SymbolValidator, MarketPhaseClassifier, OpportunityScoreCalculator,
    MemorySymbolCache, SymbolProfile, MarketPhase, PriceStructure, VolumeProfile,
    OrderBookMetricsData, PatternMetricsData, SessionMetricsData
)
...
class TestSymbolsPerformanceE2E:
    ...
    def test_concurrent_symbol_analysis_performance(self, performance_components, large_dataset) -> None:
        ...
        for thread_count in thread_counts:
            ...
            print(f"Threads: {thread_count}, Time: {total_time:.2f}s, "
    ...
    def test_cache_performance_under_load(self, performance_components, large_dataset) -> None:
        ...
        for symbol in dataset['symbols'][:50]:  # Первые 50 символов
            profile = SymbolProfile(
                ...
                market_phase=MarketPhase.BREAKOUT_ACTIVE,
                confidence=ConfidenceValue(0.7),
                volume_profile=VolumeProfile(current_volume=VolumeValue(1000.0), volume_trend=VolumeValue(0.1), volume_stability=ConfidenceValue(0.8)),
                price_structure=PriceStructure(current_price=PriceValue(market_data[symbol]['close'].iloc[-1]), atr=ATRValue(1.0), atr_percent=0.01, vwap=VWAPValue(market_data[symbol]['close'].mean()), vwap_deviation=0.001, price_entropy=EntropyValue(0.3), volatility_compression=VolatilityValue(0.2)),
                order_book_metrics=OrderBookMetrics(bid_ask_spread=SpreadValue(0.1), spread_percent=0.001, bid_volume=VolumeValue(100.0), ask_volume=VolumeValue(120.0), volume_imbalance=-0.1, order_book_symmetry=0.8, liquidity_depth=0.9, absorption_ratio=0.7),
                pattern_metrics=PatternMetrics(mirror_neuron_score=0.6, gravity_anomaly_score=0.5, reversal_setup_score=0.4, pattern_confidence=PatternConfidenceValue(0.7), historical_pattern_match=0.6, pattern_complexity=0.5),
                session_metrics=SessionMetrics(session_alignment=SessionAlignmentValue(0.8), session_activity=0.7, session_volatility=VolatilityValue(0.15), session_momentum=MomentumValue(0.6), session_influence_score=0.7)
            )
    ...
    def test_doass_performance_with_large_dataset(self, performance_components, large_dataset) -> None:
        ...
        for symbol in dataset['symbols']:
            profile = SymbolProfile(
                ...
                market_phase=np.random.choice([MarketPhase.BREAKOUT_ACTIVE, MarketPhase.EXHAUSTION, MarketPhase.ACCUMULATION]),
        ...
        for limit in limits:
            ...
            with patch.object(components['doass'], 'get_detailed_analysis') as mock_doass:
                mock_doass.return_value = Mock(
                    ...
                    detailed_profiles={symbol: components['cache'].get_profile(symbol) 
                                     for symbol in dataset['symbols'][:limit]},
            ...
            results[limit] = {
                ...
                'symbols_selected': len(analysis_result.selected_symbols)
            }
            print(f"Limit: {limit}, Time: {processing_time:.4f}s, "
                  f"Selected: {len(analysis_result.selected_symbols)}")
        # Проверяем производительность
        for limit in limits:
