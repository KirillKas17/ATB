...
class TestStrategyE2E:
    ...
    def test_complete_trading_session_e2e(self, factory, registry, realistic_market_data) -> None:
        ...
        for config in strategies_config:
            ...
            created_strategies.append({"id": strategy_id, "strategy": strategy, "config": config})
                ...
                try:
                    ...
                    if signal:
                        ...
                        registry.update_strategy_performance(
                            strategy_id=strategy_id, execution_time=execution_time, success=True, pnl=pnl
                        )
                    else:
                        # Нет сигнала
                        registry.update_strategy_performance(
                            strategy_id=strategy_id, execution_time=execution_time, success=True, pnl=Decimal("0.0")
                        )
    ...
    def test_strategy_adaptation_e2e(self, factory, registry, realistic_market_data) -> None:
        ...
        strategy_id = registry.register_strategy(
            strategy=strategy, name="Adaptive E2E Strategy", tags=["e2e", "adaptive"]
        )
        strategy.activate()
        registry.update_strategy_status(strategy_id, StrategyStatus.ACTIVE)
        # Разделяем данные на периоды с разными условиями
        periods = [(0, 50, "trending_up"), (50, 100, "trending_down"), (100, 150, "sideways"), (150, 200, "volatile")]
        period_performance = {}
        for start_idx, end_idx, period_name in periods:
            ...
            for market_data in period_data:
                try:
                    ...
                    if signal:
                        ...
                        pnl = self._simulate_trade_execution(signal, market_data)
                        registry.update_strategy_performance(
                            strategy_id=strategy_id, execution_time=0.1, success=True, pnl=pnl
                        )
                except Exception as e:
                    registry.update_strategy_performance(
                        strategy_id=strategy_id,
            ...
            period_performance[period_name] = {
                ...
                "avg_confidence": (
                    sum(s.confidence for s in period_signals) / len(period_signals) if period_signals else 0
                ),
            }
    ...
    def test_risk_management_e2e(self, factory, registry, realistic_market_data) -> None:
        ...
        for risk_level in risk_levels:
            ...
            strategy_id = registry.register_strategy(
                strategy=strategy, name=f"Risk {risk_level.title()} E2E Strategy", tags=["e2e", "risk", risk_level]
            )
            strategy.activate()
            registry.update_strategy_status(strategy_id, StrategyStatus.ACTIVE)
            risk_strategies.append({"risk_level": risk_level, "strategy_id": strategy_id, "strategy": strategy})
            ...
            for risk_info in risk_strategies:
                try:
                    ...
                    if signal:
                        pnl = self._simulate_trade_execution(signal, market_data)
                        registry.update_strategy_performance(
                            strategy_id=risk_info["strategy_id"], execution_time=0.1, success=True, pnl=pnl
                        )
                except Exception as e:
                    registry.update_strategy_performance(
    ...
    def test_multi_strategy_competition_e2e(self, factory, registry, realistic_market_data) -> None:
        ...
        for config in strategy_configs:
            ...
            strategy.activate()
            registry.update_strategy_status(strategy_id, StrategyStatus.ACTIVE)
            competition_strategies.append({"id": strategy_id, "strategy": strategy, "config": config})
