...
@pytest_asyncio.fixture
async def order_executor_agent(bybit_client):
...
@pytest_asyncio.fixture
async def meta_controller_agent():
...
@pytest_asyncio.fixture
async def bayesian_meta_controller():
...
# Тесты для RiskAgent
class TestRiskAgent:
    def test_calculate_position_size(self, risk_agent, mock_market_data):
    ...
    def test_adjust_leverage(self, risk_agent, mock_market_data):
    ...
    def test_calculate_stop_loss_take_profit(self, risk_agent, mock_market_data):
...
class TestPortfolioAgent:
    ...
    def test_rebalance_portfolio(self, portfolio_agent, mock_market_data):
...
class TestMarketRegimeAgent:
    def test_detect_regime(self, market_regime_agent, mock_market_data):
        """Тест определения рыночного режима"""
        regime, confidence = market_regime_agent.detect_regime(mock_market_data)
        assert isinstance(regime, MarketRegime)
        assert 0 <= confidence <= 1

    def test_switch_regime_logic(self, market_regime_agent, mock_market_data):
...
class TestOrderExecutorAgent:
    def test_place_order(self, order_executor_agent):
...
@pytest.mark.asyncio
async def test_meta_controller_initialization(meta_controller_agent):
...
@pytest.mark.asyncio
async def test_meta_controller_evaluate_strategies(meta_controller_agent):
...
@pytest.mark.asyncio
async def test_meta_controller_retrain_if_needed(meta_controller_agent):
...
def test_activate_best_strategy(meta_controller_agent):
...
def test_is_pair_ready_to_trade(meta_controller_agent):
...
def test_pair_manager_initialization(pair_manager):
...
def test_create_base_strategy(pair_manager):
...
def test_create_base_indicators(pair_manager):
...
def test_init_pair_structure(pair_manager):
...
def test_get_pair_status(pair_manager):
...
def test_get_active_pairs(pair_manager):
...
def test_update_pair_status(pair_manager):
...
def test_check_pair_requirements(pair_manager):
...
def test_strategy_and_config_init():
...
def test_bayesian_meta_controller_initialization(bayesian_meta_controller):
...
def test_evaluate_strategy_performance(bayesian_meta_controller):
...
def test_meta_controller_initialization(meta_controller):
    """Тест инициализации мета-контроллера"""
    assert isinstance(meta_controller, MetaController)
    assert meta_controller.market_data is not None


def test_meta_controller_evaluate_strategies(meta_controller):
    """Тест оценки стратегий"""
    strategies = meta_controller.evaluate_strategies()
    assert isinstance(strategies, list)
    assert len(strategies) > 0


def test_meta_controller_retrain_if_needed(meta_controller):
    """Тест переобучения при необходимости"""
    result = meta_controller.retrain_if_needed()
    assert isinstance(result, bool)


def test_activate_best_strategy(meta_controller):
    """Тест активации лучшей стратегии"""
    strategy = meta_controller.activate_best_strategy()
    assert isinstance(strategy, dict)
    assert "name" in strategy
    assert "performance" in strategy


def test_is_pair_ready_to_trade(meta_controller):
    """Тест проверки готовности пары к торговле"""
    is_ready = meta_controller.is_pair_ready_to_trade("BTC/USDT")
