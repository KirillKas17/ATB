...
class TestBacktester:
    @pytest.mark.asyncio
    async def test_run_backtest(self, backtester, mock_strategy, mock_market_data):
    ...
    def test_calculate_metrics(self, backtester, mock_market_data):
        """Тест расчета метрик"""
        trades = [
    ...
    @pytest.mark.asyncio
    async def test_initialize(self, backtester):
        """Тест инициализации"""
        await backtester.initialize()
        assert backtester.trading_controller is not None

    def test_save_load_results(self, backtester, tmp_path):
...
class TestMarketSimulator:
    def test_simulate_market(self, mock_market_data):
...
class TestBacktestExplainer:
    def test_generate_recommendations(self, backtest_explainer, mock_market_data):
