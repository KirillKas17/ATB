...
class TestIndicatorConfig:
    ...
    def test_indicator_config_custom(self: "TestIndicatorConfig") -> None:
        ...
        config = IndicatorConfig(rsi_period=21, macd_fast=8, macd_slow=21, bb_period=50, use_cache=False)
...
class TestMACD:
    ...
    def test_macd_creation(self: "TestMACD") -> None:
        ...
        macd = MACD(macd=macd_series, signal=signal_series, histogram=histogram_series)
...
class TestVolumeProfileData:
    ...
    def test_volume_profile_data_creation(self: "TestVolumeProfileData") -> None:
        ...
        vp_data = VolumeProfileData(poc=poc, value_area=value_area, histogram=histogram, bins=bins)
...
class TestMarketStructureData:
    ...
    def test_market_structure_data_creation(self: "TestMarketStructureData") -> None:
        ...
        ms_data = MarketStructureData(
            structure=structure, trend_strength=trend_strength, volatility=volatility, adx=adx, rsi=rsi
        )
...
class TestDefaultTechnicalAnalysisService:
    ...
    def test_calculate_macd(self, service: DefaultTechnicalAnalysisService, sample_data: pd.DataFrame) -> None:
        """Тест расчета MACD."""
        macd_result = service.calculate_macd(sample_data["close"], fast=12, slow=26, signal=9)
    ...
    def test_calculate_bollinger_bands(
        self, service: DefaultTechnicalAnalysisService, sample_data: pd.DataFrame
    ) -> None:
        """Тест расчета полос Боллинджера."""
        bb_result = service.calculate_bollinger_bands(sample_data["close"], period=20, std_dev=2.0)
    ...
    def test_calculate_market_structure(
        self, service: DefaultTechnicalAnalysisService, sample_data: pd.DataFrame
    ) -> None:
