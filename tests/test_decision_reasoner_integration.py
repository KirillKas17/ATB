...
from shared.models.trade_decision import TradeDecision
...
class TestDecisionReasonerIntegration:
    ...
    def decision_reasoner(self: "TestEvolvableMarketMakerAgent") -> Any:
        ...
        return DecisionReasoner(config=None)
    ...
    @pytest.mark.asyncio
    async def test_trading_orchestrator_decision_reasoner_integration(self: "TestDecisionReasonerIntegration") -> None:
    ...
    def test_decision_reasoner_retraining_check(self, decision_reasoner) -> None:
        ...
        decision_reasoner.trade_history = [{"confidence": 0.8, "pnl": -100.0} for _ in range(100)]
    ...
    def test_decision_reasoner_model_training(self, decision_reasoner) -> None:
        ...
        trade_results = [{"confidence": 0.8, "pnl": 100.0}, {"confidence": 0.6, "pnl": -50.0}]
        # Обучаем модели
    ...
    def test_decision_reasoner_prediction_with_confidence(self, decision_reasoner) -> None:
        ...
        # Обучаем модель
        features = pd.DataFrame(
            {"feature1": np.random.randn(50), "feature2": np.random.randn(50), "feature3": np.random.randn(50)}
        )
        targets = pd.Series(np.random.randint(0, 2, 50))
        trade_results = [{"confidence": 0.8, "pnl": 100.0}]
        decision_reasoner.train_models(features, targets, trade_results)
        # Делаем предсказание
        test_features = {"feature1": 0.5, "feature2": -0.3, "feature3": 0.8}
    ...
    def test_decision_reasoner_signal_adjustment(self, decision_reasoner) -> None:
        ...
        original_signal = {"position_size": 0.1, "stop_loss": 45000.0, "take_profit": 55000.0}
        adjusted_signal = decision_reasoner.adjust_strategy_signal(
            signal=original_signal, confidence=0.8, market_regime="trend"
        )
