...
class TestExchangesIntegration:
    ...
    def sample_market_data_request(self: "TestEvolvableMarketMakerAgent") -> Any:
        ...
        return MarketDataRequest(symbol=Symbol("BTC/USDT"), timeframe=TimeFrame.HOUR_1, limit=100)
    ...
    async def test_factory_specialized_methods(self, sample_connection_config) -> None:
        ...
        with patch("infrastructure.external_services.exchanges.bybit_exchange_service.BybitExchangeService"):
            bybit_service = ExchangeServiceFactory.create_bybit_service(
                api_key="bybit_key", api_secret="bybit_secret", testnet=True, connection_config=sample_connection_config
            )
    ...
    async def test_service_lifecycle(self, sample_credentials, sample_connection_config) -> None:
        ...
        with patch(
            "infrastructure.external_services.exchanges.bybit_exchange_service.BybitExchangeService"
        ) as mock_bybit_class:
    ...
    async def test_market_data_flow(
        self, sample_credentials, sample_connection_config, sample_market_data_request
    ) -> None:
    ...
    async def test_balance_and_positions_flow(self, sample_credentials, sample_connection_config) -> None:
        ...
        with patch(
            ...
            # Мокаем позиции
            mock_positions = [
                {"symbol": "BTC/USDT", "side": "long", "contracts": 0.1, "notional": 5000.0, "unrealized_pnl": 100.0}
            ]
    ...
    async def test_rate_limiting_integration(
        ...
        with patch(
            ...
            tasks = [service.get_market_data(sample_market_data_request) for _ in range(3)]
