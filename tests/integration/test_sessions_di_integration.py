...
class TestSessionsDIIntegration:
        ...
        container.config.from_dict(
            {
                ...
                "bybit": {"api_key": "test_key", "api_secret": "test_secret", "testnet": True},
            }
        )
        return container
    ...
    def test_session_repository_di_integration(
        self, container, session_repository_config, mock_session_analysis_result
    ) -> None:
    ...
    def test_session_predictor_di_integration(self, container, mock_market_data) -> None:
        ...
        prediction = session_predictor.predict_session_behavior(
            session_type=SessionType.ASIAN, market_conditions=mock_market_data.tail(1).to_dict("records")[0]
        )
    ...
    def test_session_optimizer_di_integration(self, container) -> None:
        ...
        optimized_params = session_optimizer.optimize_session_parameters(
            session_type=SessionType.ASIAN, current_parameters={"volatility_threshold": 0.1, "volume_threshold": 1000}
        )
    ...
    def test_full_session_workflow_di_integration(
        self, container, session_repository_config, mock_market_data, mock_session_analysis_result
    ) -> None:
        ...
        optimized_params = session_optimizer.optimize_session_parameters(
            session_type=SessionType.ASIAN, current_parameters={"volatility_threshold": 0.1}
        )
        assert isinstance(optimized_params, dict)
