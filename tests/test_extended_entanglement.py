...
from infrastructure.exchange_streams.stream_aggregator import MarketStreamAggregator
...
class TestStreamManager:
    ...
    def stream_manager(self: "TestEvolvableMarketMakerAgent") -> Any:
        ...
        return StreamManager(max_lag_ms=3.0, correlation_threshold=0.95, detection_interval=0.1)
    ...
    async def test_initialize_exchanges(self, stream_manager) -> None:
        ...
        # Мокаем клиенты
        with patch("application.entanglement.stream_manager.BingXWebSocketClient") as mock_bingx, patch(
            "application.entanglement.stream_manager.BitgetWebSocketClient"
        ) as mock_bitget, patch("application.entanglement.stream_manager.BybitWebSocketClient") as mock_bybit:
...
class TestWebSocketClients:
    ...
    async def test_integration_workflow(self) -> None:
        ...
        stream_manager = StreamManager(max_lag_ms=3.0, correlation_threshold=0.95, detection_interval=0.1)
        # Мокаем клиенты
        with patch("application.entanglement.stream_manager.BingXWebSocketClient") as mock_bingx, patch(
            "application.entanglement.stream_manager.BitgetWebSocketClient"
        ) as mock_bitget, patch("application.entanglement.stream_manager.BybitWebSocketClient") as mock_bybit:
